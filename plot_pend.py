...
returns_TD3_v4_no_discount = np.average(np.array([np.load('results/InvertedPendulum_v4_gamma_1_TD3/run_' + str(run) + '/evaluations.npz')['results'] for run in range(RUNS)]), axis=2)
...
ax.plot(steps, np.average(returns_TD3_v4, axis=0), label='v4 (\gamma = 0.99)')
ax.plot(steps, np.average(returns_TD3_v5, axis=0), label='v5 (\gamma = 0.99)')
ax.plot(steps, np.average(returns_TD3_v4_no_discount, axis=0), label='v4 (\gamma = 1)')
...
ax.fill_between(steps, np.min(returns_TD3_v4_no_discount, axis=0), np.max(returns_TD3_v4_no_discount, axis=0), alpha=0.2)
